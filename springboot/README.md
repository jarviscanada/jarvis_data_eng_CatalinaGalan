...
![create_trader_and_account_example](https://github.com/jarviscanada/jarvis_data_eng_CatalinaGalan/blob/springboot-README/springboot/src/main/resources/create_trader.png)
...
is provided via annotations and an application.properties file.
* #### Controller Layer: 
  Also called the _Presentation Layer_, this layer consists of all the app's controllers, which define 
...
* #### Service Layer:
   Also called the _Business Layer_, this layer handles all the business logic of the app, communicating 
...
* #### Repository Layer:
   Also named _Data Access layer_, this is the layer that interacts with the PostgreSQL database (in 
this case) to persist and retrieve information upon the request from the Service Layer. In the Trading 
App it consists mainly of interfaces that extend the JpaRepository interface, as well as Entity classes 
which model entities in the database and are mapped to database tables.
* #### PSQL:
...
### Swagger UI
Swagger UI is a free OpenAPI tool that allows users to interact with REST APIs. It provides visual 
representation of the API and its documentation. The Trading App can be accessed via Swagger UI, which 
...
layer was tested manually using Postman and Swagger UI.  
...
![docker_diagram](https://github.com/jarviscanada/jarvis_data_eng_CatalinaGalan/blob/springboot-README/springboot/src/main/resources/docker_diagram.png)
...
* #### catagalan/trading-psql
   The first image uses docker's postgres:9.6-alpine as its base, and it creates the databases for the 
